...
scene_data['home'] = ['home', ['BUTTON_MIDDLE_DOWN', 'JESNK_SIDE'], ['시작', '다음', '페퍼']]
    ...
    if scene == 'first_menu':
        if input_ret['type'] == 'touch':
            ...
            if input_ret['touch_position'] == 'BUTTON_RIGHT_DOWN':
                ...
                srv['aas'].say(
                    "제가 궁금하신가요? 저는 페퍼입니다. 소프트뱅크사에서 만들어진 휴머노이드 로봇으로, 인공지능을 사용할 수 있습니다. 귀여운 외모가 특징이며, 우리나라에선 금융, 서점, 의료, 유통 등 다양한 분야에 도입되어 있습니다.")
    ...
    if scene == 'tour':
        if input_ret['type'] == 'touch':
            if input_ret['touch_position'] == 'BUTTON_RIGHT':
                ...
                srv['tablet'].showWebview(get_html_address(next_scene))
                srv['aas'].say("키는 약 1미터 이며, 다양한 카메라를 통해 물체를 인식하고, 그리퍼로 집는 것이 가능한, 만능 로봇입니다. 하지만 저보단 못생겼죠?",
                               aas_configuration)

                next_scene = 'tour_blitz'
                srv['tablet'].showWebview(get_html_address(next_scene))
                srv['aas'].say(
                    "다음 로봇은, 블리츠 입니다. 물체를 옮기는데 특화된, 베이스 로봇과, 물체를 집는, 유알 파이브 로봇을 합쳐 만들어진 로봇입니다. 그외에 소리, 카메라 센서들을 탑재하여, 물체를 인식해 그리퍼로 집는 것이 가능한, 이동형 조작 로봇입니다.",
                    aas_configuration)
            ...
            if input_ret['touch_position'] == 'BUTTON_LEFT':
                ...
                srv['tablet'].showWebview(get_html_address(next_scene))
                srv['aas'].say(
                    "저희 연구실을 소개하겠습니다. 저희 바이오지능 연구실은 다음과 같은 연구들을 하고 있습니다. 먼저 베이비마인드, 브이티티 등, 인간 수준의 인공지능 개발을 위해, 인공지능, 심리학, 인지과학 등 다양한 분야와의 학제적 연구를 진행하고 있습니다. 또한 인간과 함께 활동하는 홈 로봇, 세계 로봇 대회인 로보컵 등, 다양한 플랫폼의 로봇 연구 또한 활발히 진행하고 있습니다.",
                    aas_configuration)
